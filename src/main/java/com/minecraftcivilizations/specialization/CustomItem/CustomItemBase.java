...
public abstract class CustomItemBase {
    ...
    @Getter
    protected int customModelData; // null = not used
    @Getter
...
//    public UseCooldownComponent cooldownComponent;
    ...
    public CustomItemBase(int id, String displayName, Material material) {
        this(id, displayName, material, id, -1, true, false);
    }

    // === Constructor variations ===
    public CustomItemBase(int id, String displayName, Material material, boolean uses_cooldown) {
        this(id, displayName, material, id, -1, true, uses_cooldown);
    }

    public CustomItemBase(int id, String displayName, Material material, int customModelData, boolean uses_cooldown) {
        this(id, displayName, material, customModelData, -1, true, uses_cooldown);
    }

    public CustomItemBase(int id, String displayName, Material material, boolean enabled, boolean uses_cooldown) {
     ...
     */
    public CustomItemBase(int id) {
        this(id, null, Material.WOODEN_SWORD, id, 0, true, true);
    }
    ...
    public ItemStack wrapItemStack(ItemStack item_stack, Player player) {
        ...
        if(meta.getPersistentDataContainer().has(CustomItemManager.key_custom_item_id, PersistentDataType.INTEGER)){
            Debug.broadcast("customitem", "tried re-wrapping "+ item_stack.getType()+" as "+getId()+"!!!");
            return item_stack;
        }
        meta.getPersistentDataContainer().set(CustomItemManager.key_custom_item_id, PersistentDataType.INTEGER, getId());
        ...
        if(maxStackSize>0) {
            ...
            unsafe.modifyItemStack(item_stack, "MaxStackSize=" + maxStackSize);
        }
...
//            Debug.broadcast("customitem", "cooldown created");
    ...
    public void applyCooldown(Player player, int ticks) {
        // 1.21.1 uses the Material for the cooldown
